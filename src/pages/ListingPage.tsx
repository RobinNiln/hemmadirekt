...
import { ArrowLeft, Calendar, CheckCircle2, ChevronLeft, ChevronRight, Clock, Gavel, Grid2x2, MessageSquare, ShieldCheck, Smartphone, Users } from 'lucide-react'
...
export default function ListingPage() {
  ...
  const [modal, setModal] = useState<'viewing' | 'bid' | 'contact' | null>(null)
  ...
  const facts: [string, string][] = [
    ...
    ...(listing.floor ? ([['Våning', listing.floor]] as [string, string][]) : []),
  ...
  return (
    <div className="pb-10">
      <Container className="pt-6">
        <Link to="/kopa" className="inline-flex items-center gap-1.5 text-sm font-medium text-ink-muted hover:text-ink">
          <ArrowLeft className="h-4 w-4" /> Alla bostäder
        </Link>
        {isOwn && (
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-xl bg-petrol-700 px-4 py-3 text-sm text-white">
            <span>Det här är din annons – så här ser köparna den.</span>
      ...
      <Container className="mt-5">
        <div className="relative grid gap-2 overflow-hidden rounded-3xl md:grid-cols-4 md:grid-rows-2">
          <button className="md:col-span-2 md:row-span-2" onClick={() => setLightbox(0)}>
            <Photo src={listing.images[0]} alt={listing.street} eager className="aspect-[4/3] h-full w-full md:aspect-auto md:min-h-[440px]" />
          </button>
          {listing.images.slice(1, 5).map((img, i) => (
            <button key={img + i} className="hidden md:block" onClick={() => setLightbox(i + 1)}>
              <Photo src={img} alt={`${listing.street} bild ${i + 2}`} className="h-full min-h-[216px] w-full hover:opacity-90" />
            </button>
          ))}
      ...
      <Container className="mt-10 grid gap-10 lg:grid-cols-[1fr_380px]">
        <div className="min-w-0">
          ...
          <p className="mt-6 text-3xl font-bold tracking-tight">{formatSEK(listing.price)}</p>
          ...
          <section className="mt-12">
            ...
            <ul className="mt-6 flex flex-wrap gap-2">
              {listing.highlights.map((h) => (
                <li key={h} className="rounded-full bg-sand-200 px-3 py-1.5 text-sm text-ink-soft">
                  {h}
                </li>
              ))}
            </ul>
          ...
          <section className="mt-12">
            ...
            <Card className="mt-4 flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
              ...
              <Button onClick={() => setModal('viewing')}>Boka plats</Button>
            </Card>
          </section>
        </div>
        ...
        <aside className="lg:sticky lg:top-24 lg:self-start">
          <Card className="p-6">
            ...
            <p className="mt-1 text-sm text-ink-muted">Boka visning eller lägg ett bud direkt här.</p>
            <div className="mt-5 space-y-2.5">
              <Button full size="lg" onClick={() => setModal('viewing')}>
                <Calendar className="h-5 w-5" /> Boka visning
              </Button>
              <Button full size="lg" variant="accent" onClick={() => setModal('bid')}>
                <Gavel className="h-5 w-5" /> Lägg bud
              </Button>
              <Button full variant="ghost" onClick={() => setModal('contact')}>
          ...
          <p className="mt-3 flex items-center justify-center gap-1.5 text-xs text-ink-muted">
            <Users className="h-3.5 w-3.5" /> {listing.daysOnMarket} dagar på {BRAND.name}
          </p>
        </aside>
      </Container>
